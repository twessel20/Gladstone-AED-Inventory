const VERSION='gfd-aed-pwa-v116';
...
async function decorateMainApp(response){
  ...
  if(isMainApp){
    if(!html.includes('report-pdf.css'))html=html.replace('</head>','<link rel="stylesheet" href="./report-pdf.css?v=116"></head>');
    if(!html.includes('report-builder.js'))html=html.replace('</body>','<script src="./report-builder.js?v=116"></script></body>');
