...
function healthData(units,year,quarter){let oos=0,expired=0,due30=0,due180=0,missing=0,checksComplete=0,deployments=0,shocks=0,issues=[];units.forEach(a=>{const label=a.location+(a.descriptor?' — '+a.descriptor:'');if(String(a.status||'In Service')!=='In Service'){oos++;issues.push(label+': '+(a.status||'Out of Service'))}if(periodChecks(a,year,quarter).length)checksComplete++;components(a).forEach(c=>{const d=daysTo(c.date);if(d===null){missing++;issues.push(label+': '+c.name+' expiration not recorded')}else if(d<0){expired++;issues.push(label+': '+c.name+' expired '+dateFmt(c.date))}else if(d<=30){due30++;due180++;issues.push(label+': '+c.name+' expires in '+d+' day'+(d===1?'':'s')+' ('+dateFmt(c.date)+')')}else if(d<=180){due180++}});(a.deps||[]).filter(d=>periodDate(d.date,year,quarter)).forEach(d=>{deployments++;if(d.shock)shocks++})});const critical=oos+expired,watch=due30+missing,health=critical?'CRITICAL':watch?'ATTENTION':'GOOD',healthText=critical?'One or more AEDs are out of service or have an expired component and require immediate correction.':watch?'All selected AEDs are currently serviceable, but one or more items require attention, including an expiration within 30 days or a missing expiration date.':'All selected AEDs are in service with no expired components, no missing expiration dates, and no component expirations within the next 30 days.';return {oos,expired,due30,due180,missing,checksComplete,deployments,shocks,issues:[...new Set(issues)],health,healthText}}
function executiveSummary(units,year,quarter,mode,groupName){const h=healthData(units,year,quarter),scope=mode==='group'?groupName:mode==='individual'?(units[0].location+(units[0].descriptor?' — '+units[0].descriptor:'')):'Selected AEDs',complete=h.checksComplete===units.length;const issues=h.issues.length?'<h3 style="margin:14px 0 6px">Items Requiring Attention / Watch</h3><ul style="margin:0;padding-left:22px">'+h.issues.slice(0,12).map(x=>'<li>'+safe(x)+'</li>').join('')+(h.issues.length>12?'<li>+'+(h.issues.length-12)+' additional item(s) detailed in this report.</li>':'')+'</ul>':'<p><b>Attention items:</b> None identified from the recorded inventory data.</p>';return '<section style="page-break-after:always;break-after:page"><div class="report-box"><h2 style="margin-top:0;color:#123a5a">Executive Summary</h2><p><b>Scope:</b> '+safe(scope)+' · Q'+quarter+' '+year+' · '+units.length+' AED'+(units.length===1?'':'s')+'</p><div class="report-grid"><div class="report-box"><h3 style="margin-top:0">Overall Health</h3><div class="health health-'+(h.health==='GOOD'?'good':h.health==='ATTENTION'?'watch':'attention')+'" style="font-size:22px;font-weight:800">'+safe(h.health)+'</div><p>'+safe(h.healthText)+'</p></div><div class="report-box"><h3 style="margin-top:0">Quarterly Status</h3><div class="kv"><b>Inspections completed</b><div>'+h.checksComplete+' / '+units.length+'</div></div><div class="kv"><b>Quarter complete</b><div>'+(complete?'Yes':'No')+'</div></div></div></div><div class="report-grid"><div class="report-box"><h3 style="margin-top:0">Readiness</h3><div class="kv"><b>Out of service</b><div>'+h.oos+'</div></div><div class="kv"><b>Expired components</b><div>'+h.expired+'</div></div><div class="kv"><b>Expiration ≤30 days</b><div>'+h.due30+'</div></div><div class="kv"><b>Expiration 31–180 days</b><div>'+Math.max(0,h.due180-h.due30)+'</div></div><div class="kv"><b>Missing expiration dates</b><div>'+h.missing+'</div></div></div><div class="report-box"><h3 style="margin-top:0">Activity This Quarter</h3><div class="kv"><b>Deployments</b><div>'+h.deployments+'</div></div><div class="kv"><b>Shock-delivery events</b><div>'+h.shocks+'</div></div></div></div>'+issues+'<p class="muted" style="margin-top:18px">Health is generated from the current status, recorded component expiration dates, quarterly inspection records, and deployment history contained in this report.</p></div></section>'}
...
async function saveReportPdf(source,filename){const blob=await reportPdfBlob(source,filename),url=URL.createObjectURL(blob);window.open(url,'_blank');setTimeout(()=>URL.revokeObjectURL(url),120000)}
async function shareReportPdf(source,filename,title,summary){const blob=await reportPdfBlob(source,filename),file=new File([blob],filename,{type:'application/pdf'});if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({title,files:[file],text:summary||title});return}const url=URL.createObjectURL(blob);window.open(url,'_blank');setTimeout(()=>URL.revokeObjectURL(url),120000)}
function reportSection(a,year,quarter){const checks=periodChecks(a,year,quarter),deps=(a.deps||[]).filter(d=>periodDate(d.date,year,quarter)).sort((x,y)=>String(y.date||'').localeCompare(String(x.date||'')));const audit=(db.audit||[]).filter(e=>e.id===a.id&&periodDate(e.time,year,quarter)&&/Status Change|Return|Deployment/i.test(e.type||'')).sort((x,y)=>String(y.time||'').localeCompare(String(x.time||'')));const checkRows=checks.length?checks.map(c=>'<tr><td>'+dateFmt(c.date)+'</td><td>'+safe(c.inspector||'N/A')+(c.employeeNumber?' · #'+safe(c.employeeNumber):'')+'</td><td>'+safe(c.auditResult||c.status||'Completed')+'</td></tr>').join(''):'<tr><td colspan="3">No quarterly inspection recorded.</td></tr>';const depRows=deps.length?deps.map(d=>'<tr><td>'+dateFmt(d.date)+'</td><td>'+safe(d.report||d.agencyReport||'N/A')+'</td><td>'+(d.shock?'Shock delivered':'No shock')+(d.adultPadsUsed?' · Adult pads used':'')+(d.pediatricPadsUsed?' · Pediatric pads used':'')+(d.rts?' · Returned to service':'')+'</td></tr>').join(''):'<tr><td colspan="3">No deployments recorded.</td></tr>';const auditRows=audit.length?audit.map(e=>'<tr><td>'+dateFmt(String(e.time||'').slice(0,10))+'</td><td>'+safe(e.type)+'</td><td>'+safe(e.detail||'')+'</td></tr>').join(''):'<tr><td colspan="3">No status/return-to-service events recorded.</td></tr>';return '<section class="report-box" style="margin:0 0 18px;break-inside:avoid"><h2 style="margin:0;color:#123a5a">'+safe(a.location)+(a.descriptor?' — '+safe(a.descriptor):'')+'</h2><p class="muted" style="margin:4px 0 12px">Serial: '+safe(a.serial)+' · Group: '+safe(a.group||'Unassigned')+' · Current Status: '+status(a)+'</p><div class="report-grid"><div class="report-box"><h3>Current Components</h3><div class="kv"><b>Adult Pads</b><div>'+dateFmt(a.adult)+'</div></div>'+(a.pedConfig!=='N/A'?'<div class="kv"><b>Pediatric Pads</b><div>'+dateFmt(a.ped)+'</div></div>':'')+'<div class="kv"><b>Battery</b><div>'+dateFmt(a.battery)+'</div></div></div><div class="report-box"><h3>Quarterly Inspection</h3><table class="summary-table"><thead><tr><th>Date</th><th>Inspector</th><th>Result</th></tr></thead><tbody>'+checkRows+'</tbody></table></div></div><h3>Deployments</h3><table class="summary-table"><thead><tr><th>Date</th><th>Report #</th><th>Event</th></tr></thead><tbody>'+depRows+'</tbody></table><h3>Status / Return-to-Service History</h3><table class="summary-table"><thead><tr><th>Date</th><th>Event</th><th>Details</th></tr></thead><tbody>'+auditRows+'</tbody></table></section>'}
function build(){const units=choose();if(!units.length){alert('Select at least one AED.');return}const sig=typeof overallReportSignature==='function'?overallReportSignature(true):null;if(!sig)return;const {year,quarter}=period(),mode=$id('musterMode').value,groupName=mode==='group'?($id('musterGroup')?.value||'Unassigned'):'',title=mode==='group'?groupName+' AED Report':mode==='individual'?'Individual AED History':'Selected AED Report',subtitle=mode==='group'?'Gladstone Fire / EMS · Group: '+groupName+' · Q'+quarter+' '+year+' · '+units.length+' AED'+(units.length===1?'':'s'):'Gladstone Fire / EMS · Q'+quarter+' '+year+' · '+units.length+' AED'+(units.length===1?'':'s');const area=$id('reportPrintArea');if(!area){alert('Report print area is unavailable.');return}const generated='Generated '+new Date().toLocaleString()+'.',header='<div class="report-header"><img src="gfd-patch.jpg" alt="Gladstone Fire EMS"><div class="report-title"><h1>'+safe(title)+'</h1><p>'+safe(subtitle)+'</p></div></div>',exec=executiveSummary(units,year,quarter,mode,groupName),details=units.map(a=>reportSection(a,year,quarter)).join(''),signature=typeof overallSignatureHTML==='function'?overallSignatureHTML(sig):'',core=header+exec+details+signature+'<div class="report-foot">'+safe(generated)+'</div>',slug=String(mode==='group'?groupName:title).replace(/[^a-z0-9]+/ig,'-').replace(/^-+|-+$/g,'').toLowerCase()||'aed-report',pdfFilename=slug+'-q'+quarter+'-'+year+'.pdf',summary=title+' · Q'+quarter+' '+year+' · '+units.length+' AED'+(units.length===1?'':'s');area.innerHTML='<div class="report-sheet" id="generatedReportSheet">'+core+'<div class="report-toolbar"><button type="button" class="primary" id="musterPreviewPdf">Preview PDF</button><button type="button" id="musterPdf">Open PDF</button><button type="button" id="musterSharePdf">Share PDF</button><button type="button" id="musterClose">Close</button></div></div>';area.style.display='block';document.querySelectorAll('main>.view').forEach(v=>v.style.display='none');area.scrollIntoView({behavior:'smooth',block:'start'});const sheet=$id('generatedReportSheet');$id('musterPreviewPdf').onclick=async()=>{const b=$id('musterPreviewPdf');b.disabled=true;b.textContent='Building Preview…';try{await previewGeneratedPdf(sheet,pdfFilename)}catch(e){alert(e.message||'Unable to preview PDF.')}finally{b.disabled=false;b.textContent='Preview PDF'}};$id('musterPdf').onclick=async()=>{const b=$id('musterPdf');b.disabled=true;b.textContent='Creating PDF…';try{await saveReportPdf(sheet,pdfFilename)}catch(e){alert(e.message||'Unable to create PDF.')}finally{b.disabled=false;b.textContent='Open PDF'}};$id('musterSharePdf').onclick=async()=>{const b=$id('musterSharePdf');b.disabled=true;b.textContent='Preparing PDF…';try{await shareReportPdf(sheet,pdfFilename,title,summary)}catch(e){if(!e||e.name!=='AbortError')alert(e.message||'Unable to share PDF.')}finally{b.disabled=false;b.textContent='Share PDF'}};$id('musterClose').onclick=()=>{area.style.display='none';document.querySelectorAll('main>.view').forEach(v=>v.style.display='');if(typeof goView==='function')goView('reports')}}
